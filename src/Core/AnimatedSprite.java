...
public class AnimatedSprite extends Sprite//extends AnimationTimer
{
    ...
    public AnimatedSprite(String imagepath, float fps, int totalFrames, int cols, int rows, int frameWidth, int frameHeight)
    {
        super(imagepath);
        this.fps = fps;
    ...
    public void render(GraphicsContext gc, Long now)
    {
        ...
        gc.drawImage(image, currentCol * frameWidth, currentRow * frameHeight, frameWidth, frameHeight, positionX, positionY, frameWidth, frameHeight); //(img, srcX, srcY, srcWidht, srcHeight, TargetX, TargetY, TargetWidht, TargetHeight)
    }
